...
public struct Cell: Hashable {
    ...
    public init(
        ...
        if exotic {
            basic = try BasicCell.exotic(bits: bits, refs: refs)
        } else {
            ...
            basic = BasicCell(type: .ordinary, bits: bits, refs: refs)
        }
        let precomputed = try basic.precompute()
        mask = precomputed.mask
        _depths = precomputed.depths
        _hashes = precomputed.hashes
    }
    
    public init() {
        basic = BasicCell(type: .ordinary, bits: Bitstring.empty, refs: [])
        mask = LevelMask()
    }
    
    /// Initializes a new cell with plain bytestring. This does not parse Bag-of-Cells (BoC), but uses provided data as
    /// a bitstring (byte-aligned).
...
private struct BasicCell: Hashable {
    // MARK: Properties
    ...
    func precompute() throws -> (mask: LevelMask, hashes: [Data], depths: [UInt32]) {
        ...
        for levelI in 0 ... levelMask.level {
            ...
            for c in refs {
                let childDepth: UInt32 =
                    if type == .merkleProof || type == .merkleUpdate {
                        c.depth(level: Int(levelI) + 1)
                    } else {
                        c.depth(level: Int(levelI))
                    }
...
func getRepr(bits: Bitstring, refs: [Cell], level: UInt32, type: CellType) throws -> Data {
    ...
    for c in refs {
        let childDepth: UInt32 =
            if type == .merkleProof || type == .merkleUpdate {
                c.depth(level: Int(level) + 1)
            } else {
                c.depth(level: Int(level))
            }
    ...
    for c in refs {
        let childHash: Data =
            if type == .merkleProof || type == .merkleUpdate {
                c.hash(level: Int(level) + 1)
            } else {
                c.hash(level: Int(level))
            }
...
func exoticPruned(bits: Bitstring, refs: [Cell]) throws -> ExoticPruned {
    ...
    if bits.length == 280 {
        ...
        let level = try UInt32(reader.loadUint(bits: 8))
    ...
    for _ in 0 ..< mask.level {
        ...
        let depth = try UInt32(reader.loadUint(bits: 16))
...
func resolvePruned(
    ...
    -> (type: CellType, depths: [UInt32], hashes: [Data], mask: LevelMask) {
...
func resolveMerkleProof(
    bits: Bitstring,
    refs: [Cell]
) throws
    -> (type: CellType, depths: [UInt32], hashes: [Data], mask: LevelMask) {
    _ /* merkleProof */ = try exoticMerkleProof(bits: bits, refs: refs)
    let depths = [UInt32]()
    let hashes = [Data]()
...
func resolveMerkleUpdate(
    bits: Bitstring,
    refs: [Cell]
) throws
    -> (type: CellType, depths: [UInt32], hashes: [Data], mask: LevelMask) {
    _ /* merkleUpdate */ = try exoticMerkleUpdate(bits: bits, refs: refs)
    let depths = [UInt32]()
    let hashes = [Data]()
...
@discardableResult
func exoticMerkleUpdate(
    bits: Bitstring,
    refs: [Cell]
) throws
    -> (proofDepth1: UInt32, proofDepth2: UInt32, proofHash1: Data, proofHash2: Data) {
    let reader = Slice(bits: bits)
    ...
    let proofDepth1 = try UInt32(reader.loadUint(bits: 16))
    let proofDepth2 = try UInt32(reader.loadUint(bits: 16))

    if proofDepth1 != refs[0].depth(level: 0) {
        throw TonError
            .custom("Merkle Update cell ref depth must be exactly \(proofDepth1), got \(refs[0].depth(level: 0))")
    }

    if proofHash1 != refs[0].hash(level: 0) {
        throw TonError
            .custom(
                "Merkle Update cell ref hash must be exactly \(proofHash1.hexString()), got \(refs[0].hash(level: 0).hexString())"
            )
    }

    if proofDepth2 != refs[1].depth(level: 0) {
        throw TonError
            .custom("Merkle Update cell ref depth must be exactly \(proofDepth2), got \(refs[1].depth(level: 0))")
    }

    if proofHash2 != refs[1].hash(level: 0) {
        throw TonError
            .custom(
                "Merkle Update cell ref hash must be exactly \(proofHash2.hexString()), got \(refs[1].hash(level: 0).hexString())"
            )
    }
...
func exoticMerkleProof(bits: Bitstring, refs: [Cell]) throws -> (proofDepth: UInt32, proofHash: Data) {
    ...
    let proofDepth = try UInt32(reader.loadUint(bits: 16))
    ...
    if proofHash != refHash {
        throw TonError
            .custom("Merkle Proof cell ref hash must be exactly \(proofHash.hexString()), got \(refHash.hexString())")
    }
