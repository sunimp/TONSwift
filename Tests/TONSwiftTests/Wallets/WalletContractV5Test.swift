...
final class WalletContractV5Test: XCTestCase {
    ...
    func testBeta() throws {
        let contractBeta = WalletV5Beta(
            workchain: 0,
            publicKey: publicKey,
            walletID: WalletIDBeta(networkGlobalID: -239, workchain: 0)
        )
        
        XCTAssertEqual(
            try contractBeta.address(),
            try Address.parse("UQCRix440npsvDU88REZ8uUJ4jedPEiX_QlCgi954nhZUrBP")
        )
        XCTAssertEqual(
            try contractBeta.stateInit.data?.toString(),
            "x{000000007FFFFF888000000000002BAA432F436856F08CC980DDD818CD12F6B5894E25852BF947B1224D9EFCE2912_}"
        )
        XCTAssertEqual(
            try contractBeta.stateInit.code?.toString(),
            "x{02E4CF3B2F4C6D6A61EA0F2B5447D266785B26AF3637DB2DEEE6BCD1AA826F3412}"
        )
        
        let transferMultiple = try contractBeta.createTransfer(args: argsMultiple())
        let signedDataMultiple = try transferMultiple
            .signMessage(signer: WalletTransferSecretKeySigner(secretKey: secretKey))
        ...
        let transferSingle = try contractBeta.createTransfer(args: argsSingle())
        let signedDataSingle = try transferSingle
            .signMessage(signer: WalletTransferSecretKeySigner(secretKey: secretKey))
    ...
    func testR1() throws {
        let contractR1 = WalletV5R1(
            workchain: 0,
            publicKey: publicKey,
            walletID: WalletID(networkGlobalID: -239, workchain: 0)
        )
        ...
        XCTAssertEqual(
            ...
            "x{800000003FFFFF88ABAA432F436856F08CC980DDD818CD12F6B5894E25852BF947B1224D9EFCE2912_}"
        )
        XCTAssertEqual(
        ...
        let transferMultiple = try contractR1.createTransfer(args: argsMultiple())
        let signedDataMultiple = try transferMultiple
            .signMessage(signer: WalletTransferSecretKeySigner(secretKey: secretKey))
        let cellMultiple = try Cell(data: signedDataMultiple)
        ...
        let transferSingle = try contractR1.createTransfer(args: argsSingle())
        let signedDataSingle = try transferSingle
            .signMessage(signer: WalletTransferSecretKeySigner(secretKey: secretKey))
        let cellSingle = try Cell(data: signedDataSingle)
