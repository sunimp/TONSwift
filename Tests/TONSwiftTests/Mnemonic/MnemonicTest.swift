...
final class MnemonicTest: XCTestCase {
    func testMnemonic() throws {
        ...
        let mnemonicArray =
            "cluster notice abandon frost gospel boring element situate click mix vague replace imitate garment useful crater resource dose tenant theme foam ancient phrase slight"
    ...
    func testIsMultiAccountSeedMnemonic() throws {
        let collisionMnemonic =
            "cluster notice abandon frost gospel boring element situate click mix vague replace imitate garment useful crater resource dose tenant theme foam ancient phrase slight"
                .components(separatedBy: " ")
    
        XCTAssertFalse(Mnemonic.isMultiAccountSeed(mnemonicArray: collisionMnemonic))
    
        let multiAccountMnemonic =
            "execute peanut please demise thumb mango argue cloud reopen upset also dentist panic elite roast security pyramid extra boil execute lazy pledge notice check"
                .components(separatedBy: " ")

        XCTAssertTrue(Mnemonic.isMultiAccountSeed(mnemonicArray: multiAccountMnemonic))
    
        let tonMnemonic =
            "item supply cover volcano satisfy window custom cupboard license dance record tissue gadget rural health blossom useless useless hungry brush grief stock reflect morning"
                .components(separatedBy: " ")
